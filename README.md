...
- Keep the `docker` label on the PR while you test. Every new successful CI run rebuilds the preview image automatically.
- If you add the label after CI already finished, push a new commit or rerun CI so the preview workflow gets a fresh CI completion event.
- Each successful CI run refreshes the same `pr-123` tag, so you can keep testing the latest branch state without re-labeling the PR.
