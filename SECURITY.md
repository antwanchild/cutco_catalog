...
- **Traefik + authentik friendly** — if you terminate auth at the edge, forward a trusted username header and configure `TRUSTED_AUTH_USERNAME_HEADER` to match. The app treats that request as authenticated. The legacy `AUTHENTIK_USERNAME_HEADER` setting still works if you already used it.
...
- [ ] If using Traefik + authentik, forward the authenticated username into `X-Forwarded-User` or your chosen trusted header
