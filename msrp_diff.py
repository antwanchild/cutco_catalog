...
def scrape_live_prices(
    ...
    *,
    ...
    if include_catalog:
        print(
            "Catalog rediscovery is disabled for MSRP diff; using stored DB URLs.",
            flush=True,
        )
    if db_items is None:
        app = create_app()
    ...
    fetched, timed_out = _fetch_live_prices_by_sku(
        by_sku, workers=workers, log_fn=lambda msg: print(msg, flush=True)
    )
    if timed_out:
...
def write_csv(diff: dict, path: str) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=["category", "sku", "name", "db_price", "live_price", "delta"],
        )
        writer.writeheader()
...
def post_discord(diff: dict, webhook_url: str) -> None:
    ...
    if new_items:
        ...
        for row in new_items[:5]:
            lines.append(
                f"  • {row['name']} (#{row['sku']}) — {_fmt_price(row['live_price'])}"
            )
        if len(new_items) > 5:
...
def main() -> None:
    ...
    with app.app_context():
        ...
        if args.update or args.repair_stale:
            ...
            mode = (
                "stale MSRP rows"
                if args.repair_stale and not args.update
                else "MSRP prices"
            )
